...
嗯，我用的 [IdGenerator](https://github.com/yitter/IdGenerator), 递增的魔改雪花算法，感觉还行
...
MySQL 所独有的一种字符串类型。**抛出的错误提示也并不直观**，这样的实现总有一些遗憾，主要是因为MySQL 8.0 之前的版本并没有提供约束功能。
...
虽然 Mybatis Plus 很好用，但请不要完全依赖Service，而碰都不碰Dao

Service可以用来校验数据合法，用来调用多个Mapper，聚合处理......但不要让它身兼Dao的工作

这样你的Service层和Dao层是强耦合的，做个SQL优化，可能还牵扯到原本的业务逻辑；业务逻辑变更，没准还要频繁改动SQL语句......不要这样！！！

真正好的分层，Dao层变动是不影响Service层调用的。Service就只调Dao，而不会去调另一个Service。否则耦合的东西多了，牵一发而动全身

要明确的是：Dao层是不经常改动的，因为他是偏底层的，一般来说大把Service要调它。**底层的东西，不要经常动！！！否则等着被别人线下真人快打吧**
